/*
 * Modo UART del SIO del GBA. Registros:
 *   REG_RCNT   = 0           (no GPIO; deja que SIOCNT controle)
 *   REG_SIOCNT = bits...
 *
 * Bits relevantes de SIOCNT en modo UART:
 *   bits 0-1 : baud (00=9600, 01=38400, 10=57600, 11=115200)
 *   bit 2    : CTS  (0 = enviar siempre)
 *   bit 3    : Parity Control
 *   bit 4    : SI / Send Data Flag (0 = TX no lleno)
 *   bit 5    : SO / Receive Data Flag (1 = RX vacío)
 *   bit 6    : Error Flag
 ...
 *   bits 12  : ?
 *   bit 13   : Mode select (debe ser 1 0 = UART → SIO_MODE_UART)
 *   bit 14   : IRQ enable
 *   bit 15   : Mode bit 1
 *
...
/* libgba/gba_sio.h no define los bits específicos del modo UART; los
 * añadimos aquí según la documentación oficial del GBA (gbatek). */
#define SIO_UART_CTS         (1u <<  2)
#define SIO_UART_PARITY_ODD  (1u <<  3)
#define SIO_UART_TX_FULL     (1u <<  4)   /* read-only: 1 = TX full */
#define SIO_UART_RX_EMPTY    (1u <<  5)   /* read-only: 1 = RX vacío */
...
/* Busy-spin sin VBlankIntrWait. ~280k iter por frame (16ms) a 16.78MHz CPU.
 * Drena el FIFO al ritmo del CPU, mucho mas rapido que 115200 baud
 * (~11 KB/s), por lo que el FIFO de 4 bytes nunca se llena. */
int uart_recv_byte_busy(u32 frame_equivalent) {
    /* Cap superior conservador: ~280k spins/frame. */
